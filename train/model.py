...
def createModel():
    ...
    gruMessage = tf.keras.layers.GRU(256, return_sequences=True, name="gruMessage")(inputMessage)
    gruValue = tf.keras.layers.GRU(256, return_sequences=True, name="gruValue")(inputValue)
    gruDT = tf.keras.layers.GRU(256, return_sequences=True, name="gruDT")(inputDT)

    x = tf.keras.layers.concatenate([gruMessage, gruValue, gruDT], name="concatenate")
    gru = tf.keras.layers.GRU(1024, return_sequences=True, name="gruGlobal")(x)
